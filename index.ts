...
const filteredarr=numArr1.filter((val:number)=>{
    if(val>0){
    return true;
}
});
...
const mappedarr=numArr2.map((val:number)=>{
    val*=2;
    return val;
}
);
...
const filterredstr=strArr1.filter((array:string)=>{
...
const convert=numArr4.map((val:number)=>{
...
const doubledvalue = (arr: number[]): number[] => {
    return arr.filter(val => val % 2 != 0)
              .map(val => val + val);     
};
...
const modifiedlist=strArr2.forEach((array:string)=>{
    console.log(`${array}!`);
}
);